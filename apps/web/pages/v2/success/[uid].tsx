...
export default function Success(props: SuccessProps) {
  ...
  const bookingInfo = JSON.parse(props.bookingInfo);

  const [date, setDate] = useState(dayjs.utc(asStringOrThrow(bookingInfo.startTime)));
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const eventTypeId = bookingInfo?.eventTypeId;

  const eventTypeRaw = !eventTypeId ? getDefaultEvent("15min") : await getEventTypesFromDB(eventTypeId);
  if (!eventTypeRaw) {
    return {
      notFound: true,
    };
  ...
  return {
    props: {
      ...
      dynamicEventName: eventType.title,
